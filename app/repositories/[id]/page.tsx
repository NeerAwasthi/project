...
export default async function RepositoryDetailsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const repo = demoRepositories.find((entry) => entry.id === id) ?? demoRepositories[0];
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        <div className="rounded-3xl border border-white/10 bg-slate-900/80 p-6">
          ...
          <div className="space-y-4 text-sm text-slate-300">
            ...
            <p><span className="font-medium text-white">Language:</span> {repo.language}</p>
            <p><span className="font-medium text-white">Visibility:</span> {repo.visibility}</p>
            <p><span className="font-medium text-white">Default branch:</span> {repo.defaultBranch}</p>
        ...
        <div className="rounded-3xl border border-white/10 bg-slate-900/80 p-6">
          ...
          <div className="space-y-4">
            {[
              "Refine auth flow and cache design",
              "Add telemetry normalization for streaming events",
              "Improve dark-mode token contrast and motion timing",
            ].map((entry, index) => (
              <div key={entry} className="rounded-2xl border border-white/10 bg-slate-950/40 p-4 text-sm text-slate-300">
                <div className="font-medium text-white">#{index + 1} {entry}</div>
                <div className="mt-2 text-xs text-slate-400">2025-08-0{index + 9}</div>
              </div>
            ))}
          </div>
        </div>
